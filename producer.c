...
// producer - the GPU simulation process
...
int main(int argc, char *argv[]) {
    ...
    shmfd1 = shm_open(SHMOBJ_PATH1, O_CREAT | O_TRUNC | O_RDWR, S_IRWXU | S_IRWXG);
    if (shmfd1 < 0) {
        perror("In shm_open() of buffer 1.");
        exit(1);
    }

    shmfd2 = shm_open(SHMOBJ_PATH2, O_CREAT | O_TRUNC | O_RDWR, S_IRWXU | S_IRWXG);
    if (shmfd2 < 0) {
    ...
    shmptr1 = (float *)mmap(NULL, shared_seg_size, PROT_READ | PROT_WRITE, MAP_SHARED, shmfd1, 0);
    if (shmptr1 == NULL) {
        perror("In mmap() requesting buffer 1.");
        exit(1);
    }
    
    shmptr2 = (float *)mmap(NULL, shared_seg_size, PROT_READ | PROT_WRITE, MAP_SHARED, shmfd2, 0);
    if (shmptr2 == NULL) {
    ...
    if (munmap(shmptr1, shared_seg_size) != 0) {
        perror("In munmap() of buffer 1");
        exit(1);
    }

    if (munmap(shmptr2, shared_seg_size) != 0) {
        perror("In munmap of buffer 2");
        exit(1);
    }
